...
class AcceptDossierUseCase:
    ...
    async def execute(self, dossier_id: str, editor_id: int, note: str | None):
        ...
        dossier.accept(editor_id=editor_id, note=note)
        ...
        if manuscript is not None:
            await self.notification_port.send(
                ...
                subject="Manuscrit accepté",
                message=f"Votre manuscrit '{manuscript.title}' a été accepté.",
            )
