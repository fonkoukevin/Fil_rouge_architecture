...
class RequestChangesUseCase:
    ...
    async def execute(self, dossier_id: str, editor_id: int, note: str):
        ...
        dossier.request_changes(editor_id=editor_id, note=note)
        ...
        if manuscript is not None:
            await self.notification_port.send(
                ...
                subject="Corrections demandées",
                message=f"Des corrections ont été demandées pour votre manuscrit '{manuscript.title}'.",
            )
